...
diretorio = (get_absolute_file_path('MODELO_QUADCOPTER_NEWTON.sce'))
...
m = 0.468;      // Massa do quadrirrotor [kg]
l = 0.225;      // Distância rotor x centro de massa [m]
b = 1.140e-7;   // Coeficiente de arrasto da estrutura
k = 2.980e-6;   // Coeficiente de empuxo das hélices
Ir = 3.357e-5;  // Momento de inércia do rotor [kg*m^2]
ixx = 4.856e-3; // Momento de inércia em x [kg*m^2]
iyy = 4.856e-3; // Momento de inércia em y [kg*m^2]
izz = 8.801e-3; // Momento de inércia em z [kg*m^2]
ax = 0.25;      // Coeficiente de força de arrasto em x [kg/s]
ay = 0.25;      // Coeficiente de força de arrasto em y [kg/s]
az = 0.25;      // Coeficiente de força de arrasto em z [kg/s]

//*************************************
// EXECUÇÃO DO DIAGRAMA DE BLOCOS
//*************************************

importXcosDiagram(diretorio + "MODELO_QUADCOPTER_NEWTON.zcos") // Importa diagrama
